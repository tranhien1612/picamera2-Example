...
######################################### Pi Camera #################################################################
    ...
    def start_record_video(self):
        if(self.isRecording == False):
            ...
            self.picam2.start_encoder(self.encoder, output, name="lores")
    ...
    def start_stream_udp(self, host, port):
        ...
        self.picam2.start_recording(H264Encoder(), output, name="main"); #H264Encoder(bitrate=10000000)
        print("output", output)
    ...
    def handle_messgae(self, client_socket, msg):
        ...
        if(len(msg) >= 26 and msg[0:8] == "fe 55 00"):
            ...
            if(cmdId == "02 24" and status == "01"):
                ...
                self.zoom(zoomLevel);
                data = [0xfe, 0x55, 0x01, 0x02, 0x22, num, 0x02, 0x00, 0x00, 0x25]
                client_socket.sendall(bytes(data))
            elif(cmdId == "ff ff"):
                if(msg[21:23] == "00"):
                ...
                if(self.modeCamera == False):
                    self.modeCamera = True;
                else:
                    self.modeCamera = False;
    
######################################### IR Camera #################################################################
    irCapture = None;
    modeCamera = False; #False: rgb, True: ir
            
    def irCamera_Init(self):
        self.picam2.pre_callback = self.handle_camera
        camera_id = "/dev/video2"
        self.irCapture = cv2.VideoCapture(camera_id, cv2.CAP_V4L2);
        
    def handle_camera(self, request):
        with MappedArray(request, "main") as m:
            if(self.irCapture.isOpened() and self.modeCamera == True): #self.irCapture.isOpened() and
                _, frame = self.irCapture.read();
                img = cv2.resize(frame, (m.array.shape[1], m.array.shape[0]))
                #print(f"frame: {m.array.shape}, img: {img.shape}, {m.array.shape[0]}, {m.array.shape[1]}")
                m.array[0:img.shape[0], 0:img.shape[1], 0:img.shape[2]] = img
